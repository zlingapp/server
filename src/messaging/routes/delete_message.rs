...
pub async fn delete_message(
    ...
    let (guild_id, channel_id, message_id) = path.into_inner();

    if let Ok(message) = db.get_message(&guild_id, &channel_id, &message_id).await {
        ...
        if !db
            .can_user_view_messages_in(&token.user_id, &guild_id, &channel_id)
            .await
            .unwrap()
        {
        ...
        if message.author.id != token.user_id {
            ...
            if !db
                .can_user_manage_messages(&token.user_id, &guild_id, &channel_id)
                .await
                .unwrap()
            {
                return Err(ErrorForbidden("access_denied"));
        ...
        ecm.notify_message_deleted(&channel_id, &message.id).await;
    } else {
